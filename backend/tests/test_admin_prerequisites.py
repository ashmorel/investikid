...
async def test_create_module_with_prerequisites(admin_client):
    r1 = await admin_client.post("/admin/modules", json={
    ...
    r2 = await admin_client.post("/admin/modules", json={
...
async def test_create_module_self_reference_rejected(admin_client):
    r1 = await admin_client.post("/admin/modules", json={
    ...
    r2 = await admin_client.put(f"/admin/modules/{mod_id}", json={
...
async def test_create_module_nonexistent_prerequisite_rejected(admin_client):
...
async def test_list_modules_includes_new_fields(admin_client):
    await admin_client.post("/admin/modules", json={
        "topic": "savings", "title": "Age Test", "icon": "🏦",
        "order_index": 0, "min_age": 8, "max_age": 12,
    })

    r = await admin_client.get("/admin/modules")
