...
_PATH = "/internal/weekly-digest/run"
...
async def test_200_runs_when_secret_matches(client, monkeypatch):
    ...
    async def fake_run(session):
        called["yes"] = True
        return {"sent": 1, "skipped_quiet": 0, "skipped_recent": 0, "skipped_opt_out": 0}

    monkeypatch.setattr(internal.digest_service, "run_weekly_digests", fake_run)
    r = await client.post(_PATH, headers={"X-Cron-Secret": "s3cr3t"})
    assert r.status_code == 200
    body = r.json()
    assert called.get("yes") is True
    assert body == {"sent": 1, "skipped_quiet": 0, "skipped_recent": 0, "skipped_opt_out": 0}
