...
async def test_badge_crud_lifecycle(admin_client):
    """Create → read → update → delete a badge."""
    # Create
    resp = await admin_client.post("/admin/badges", json={
        "name": "First Steps",
    ...
    # List
    resp = await admin_client.get("/admin/badges")
    assert resp.status_code == 200
    badges = resp.json()
    assert any(b["id"] == str(badge_id) for b in badges)

    # Update
    resp = await admin_client.put(f"/admin/badges/{badge_id}", json={
        "name": "Updated Badge",
        "condition_value": 5,
    })
    assert resp.status_code == 200
    ...
    # Delete
    resp = await admin_client.delete(f"/admin/badges/{badge_id}")
    assert resp.status_code == 200
...
async def test_badge_update_partial(admin_client):
    ...
    # Cleanup
    await admin_client.delete(f"/admin/badges/{badge_id}")


async def test_badge_list_empty_returns_empty_list(admin_client):
    ...
    for b in initial_badges:
        ...
        await admin_client.delete(f"/admin/badges/{b['id']}")
    ...
    resp = await admin_client.delete(f"/admin/badges/{temp_id}")
    assert resp.status_code == 200


async def test_badge_delete_with_users_returns_conflict(admin_client):
...
async def test_challenge_crud_lifecycle(admin_client):
    ...
    # Create
    resp = await admin_client.post("/admin/challenges", json={
    ...
    # List
    resp = await admin_client.get("/admin/challenges")
    assert resp.status_code == 200
    challenges = resp.json()
    assert any(c["id"] == str(challenge_id) for c in challenges)

    # Update
    resp = await admin_client.put(f"/admin/challenges/{challenge_id}", json={
    ...
    # Delete
    resp = await admin_client.delete(f"/admin/challenges/{challenge_id}")
    assert resp.status_code == 200
    assert resp.json()["status"] == "ok"
...
async def test_challenge_with_badge_id(admin_client):
    ...
    # Cleanup
    challenge_id = challenge["id"]
    await admin_client.delete(f"/admin/challenges/{challenge_id}")
    await admin_client.delete(f"/admin/badges/{badge_id}")


async def test_challenge_update_partial(admin_client):
...
async def test_challenge_premium_flag(admin_client):
...
async def test_countries_endpoint_returns_list(admin_client):
    """Countries endpoint returns a sorted list of country codes."""
    resp = await admin_client.get("/admin/countries")
    assert resp.status_code == 200
    countries = resp.json()
...
async def test_countries_endpoint_with_users(admin_client, db_session):
...
async def test_badge_not_found_returns_404(admin_client):
    """Updating non-existent badge returns 404."""
    fake_id = uuid.uuid4()
    resp = await admin_client.put(f"/admin/badges/{fake_id}", json={
        "name": "Updated",
    })
    assert resp.status_code == 404


async def test_challenge_not_found_returns_404(admin_client):
    """Updating non-existent challenge returns 404."""
    fake_id = uuid.uuid4()
    resp = await admin_client.put(f"/admin/challenges/{fake_id}", json={
        "title": "Updated",
    })
    assert resp.status_code == 404


async def test_challenge_all_types(admin_client):
    ...
    for i, challenge_type in enumerate(types):
        ...
        # Cleanup
        await admin_client.delete(f"/admin/challenges/{challenge['id']}")
