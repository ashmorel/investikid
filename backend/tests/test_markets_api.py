...
async def test_list_markets(client):
    ...
    assert by_code["GB"]["is_selected"] is True
...
async def test_switch_active_market_lazy_enrolls(client):
    ...
    assert rows["US"]["is_selected"] is True
...
async def test_list_markets_unauthenticated(client):
    r = await client.get("/markets")
    assert r.status_code == 401


async def test_switch_active_market_unauthenticated(client):
    # CSRF middleware fires before auth on POST endpoints, so 403 is also acceptable.
    r = await client.post("/me/active-market", json={"market_code": "US"})
    assert r.status_code in (401, 403)


async def test_my_market_progress_unauthenticated(client):
    r = await client.get("/me/markets")
    assert r.status_code == 401
