...
def _system_prompt(market_code: str, brief_json: dict, gap_note: str = "") -> str:
    ...
    return (
        ...
        f"SPREAD within a level: a level's concepts MUST cover DIFFERENT facets of the topic. "
        f"Do NOT stack three or more concepts about the same product, account type or "
        f"mechanism in one level (e.g. avoid a level that is mostly Roth-IRA/ISA concepts) — "
        f"spread across distinct real-world angles (earning, recording, rights, comparing, "
        f"deciding, etc.).\n\n"
        f"{gap_note}"
