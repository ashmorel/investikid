...
async def approve_level_drafts(session: AsyncSession, level: Level, *, replace: bool) -> dict:
    ...
    # Resolve concept slugs globally (slug is UNIQUE across all topics, so no
    # topic scoping is needed — and free-form module topics would fail scoped lookup).
    replaced = 0
    if replace:
    ...
    for i, d in enumerate(safe, start=1):
        concept_id = None
        if d.concept_slug:
            concept_id = await resolve_slug_global(session, d.concept_slug)
            if concept_id is None:
                logger.info("concept_unmapped slug=%s", d.concept_slug)
        session.add(Lesson(
