...
from app.services.entitlements import set_admin, set_premium
...
async def _grant_admin(argv: list[str]) -> int:
    args = [a for a in argv if a != "--revoke"]
    revoke = "--revoke" in argv
    if len(args) != 1:
        print("usage: python -m app.cli grant-admin <email|username> [--revoke]",
              file=sys.stderr)
        return 2
    ident = args[0].lower().strip()
    gen = _session_scope()
    ...
    try:
        ...
        if user is None:
            print(f"user not found: {ident}", file=sys.stderr)
            return 2
        changed = await set_admin(
            session, user, value=not revoke, actor="cli"
        )
        await session.commit()
        verb = "revoked" if revoke else "granted"
        print(f"{verb} admin for {user.username}" if changed
              else f"no-op ({user.username} already {'non-admin' if revoke else 'admin'})")
        return 0
    finally:
        await gen.aclose()
...
async def run(argv: list[str]) -> int:
    if not argv or argv[0] not in {"purge-accounts", "grant-premium", "grant-admin"}:
        print(
            "usage: python -m app.cli"
            " {purge-accounts | grant-premium <email|username> [--revoke]"
            " | grant-admin <email|username> [--revoke]}",
            file=sys.stderr,
        )
        return 2
    if argv[0] == "grant-premium":
        return await _grant_premium(argv[1:])
    if argv[0] == "grant-admin":
        return await _grant_admin(argv[1:])
    gen = _session_scope()
    session = await gen.__anext__()
    try:
