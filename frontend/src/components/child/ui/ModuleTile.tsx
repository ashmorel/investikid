...
export function ModuleTile({ emoji, title, subtitle, accent, tint, to, locked, recommended }: Props) {
  ...
  if (to && !locked) return <Link to={to} className={cn(cls, 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500')} style={{ backgroundColor: tint }}>{inner}</Link>;
