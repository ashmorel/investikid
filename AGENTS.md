...
The **"Yasmin's Choice" rebrand** programme is in progress. **`docs/superpowers/PROGRESS.md` is the single source of truth** for status and the next step. Currently: SP-0/A/B/C/D1/D2/F + the **country switcher** shipped (CI green, 6 jobs incl. an iOS Capacitor simulator build). The country switcher landed as **two independent settings** — a `content_region` learning-region focus (US/GB/HK) that gates content + default currency, plus a child-editable practice currency; the legal `country_code` is untouched and the simulator features the chosen region's exchange first. **Next = SP-E (parent/admin polish)** — brainstorm → spec → plan → execute. Workflow + full status in `docs/superpowers/PROGRESS.md`.
