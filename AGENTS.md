...
**Child app routes (as of 2026-06-23):** `/home`, `/learn`, `/market` (market picker), `/markets` (legacy), `/revise`, `/revise/session`, `/simulator`, `/shop`, `/arcade` (Penny's Arcade hub — **new Phase 1**), `/arcade/quiz-rush` (Quiz Rush — **new Phase 1**), `/progress`, `/stats`, `/settings`. Phase 2 will add `/arcade/money-word`.
