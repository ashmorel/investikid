...
| Video-health periodic trigger | ✅ shipped (code) — **needs 3 secrets** | Replaced the fragile separate Railway cron with a secret-guarded `POST /internal/video-health/run` (CSRF-exempt, `X-Cron-Secret` constant-time check, `503` when `CRON_SECRET` unset) that reuses `app.video_health.run.run`. A daily GitHub Actions workflow `.github/workflows/video-health-cron.yml` (06:00 UTC + manual "Run workflow") curls it. Spec/plan `docs/superpowers/{specs,plans}/2026-06-05-video-health-trigger-endpoint*`. **USER: set `CRON_SECRET` on the Railway backend + add a GitHub repo Actions secret `CRON_SECRET` (same value); the backend URL is hardcoded in the workflow (public). Then Actions → Video health cron → Run workflow to test. Delete the old separate Railway cron service.** On-demand "Check now" in Admin → Video health works regardless. |
