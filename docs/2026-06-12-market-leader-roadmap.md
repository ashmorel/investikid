...
**M9 · Weekly group challenges + seasonal events. ✅ BUILD DONE (2026-06-12, on `testing`) — ROADMAP BUILD TRACK COMPLETE.** Spec/plan `docs/superpowers/{specs,plans}/2026-06-12-group-challenges-events*`. Co-op group challenges: `challenges.scope` (admin-pickable in ChallengeForm), members' progress sums to one shared target, race-safe once-only rewards for every member (via `record_xp`, so coins/daily-goal ride along), `GET /groups/challenges` + group-goal progress bars on Stats. Seasonal events: AppSetting-driven (`seasonal_event` JSON — authored in /admin Settings, no deploy), child Home banner only while active, lesson-XP bonus (half-up rounding), event badge via existing badge CRUD. Migration `d1e2f3a4b5c6` (scratch-replay validated pre-push). Original scope: On top of parent-mediated leaderboard groups: a weekly co-op group challenge ("group completes 20 lessons") and an admin-authorable seasonal event frame (themed week + bonus XP + special badge). Same COPPA spine — no child-initiated social, no messaging.
