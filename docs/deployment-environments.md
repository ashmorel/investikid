...
- **`CRON_SECRET` is coupled** — change it in **two places at once**: Railway **production** env var **and** the GitHub **repo Actions secret** `CRON_SECRET` (same value). Test via Actions → "Video health cron" → Run workflow (expect HTTP 200).
  - ⚠️ **GitHub Actions secret `CRON_SECRET` exists** in `ashmorel/investikid` but currently holds the **testing** backend's value (set during the 2026-06-07 testing validation). **At cutover, set BOTH to a prod value** (recommended: rotate — generate a new `openssl rand -hex 32`, set it on **production** Railway `CRON_SECRET` *and* overwrite the GitHub secret with the same value). DB-snapshot first (changing the prod Railway var redeploys + runs `alembic upgrade head`).
  - ✅ The cron workflow on **both `main` and `testing`** now reads `BACKEND_URL` from a **repo Actions variable** (`${{ vars.BACKEND_URL || '<hardcoded fallback>' }}`; main commit `d8bef57`, testing `8622920`). The scheduled run executes from `main`, which now honors the variable.
  - ✅ **`vars.BACKEND_URL` is set to the prod cutover target** `https://investikid.up.railway.app` (the Invest-Ed → production → InvestiKid service; set 2026-06-07). **Confirm this is the final prod host at cutover** (a custom domain or `investikid-production` rename would need `gh variable set BACKEND_URL --body <prod-url>`).
